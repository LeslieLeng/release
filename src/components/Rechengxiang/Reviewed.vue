              ...
              <table id="otherAttr">
                ...
                <tr>
                  <td width="220px" id="viewerid">
                    <div  style="padding: 0;margin: 0" id="showimages">
                    </div>
                  </td>
                  <td></td>
    ...
    methods: {
      ...
      getdoc(){
       ...
       let data = this.$refs.tree.getCheckedNodes()
        ...
        // console.log('数组'+arr)
        axios.post(this.$store.state.url+'/myapp/genFile/', {
          id: arr,
        }).then((result) => {
          window.open(this.$store.state.url+'/myapp/export_docx/');
        }).catch((err) => {
          console.log(err)
        })
      },
      ...
      //获取数据
      getInfo() {
        axios.post(this.$store.state.url+'/myapp/show_info_pass/', {
          model: this.df_model,
